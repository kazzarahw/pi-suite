...
4. **Search & fetch are `browser` actions, not separate tools.** `action:"read"` (optionally with a `url`) is fetch. `action:"search"` is a one-call convenience over a **fallback chain of keyless engines** (no provider config): `read https://html.duckduckgo.com/html/?q=<query>` first, then **Bing**, with a `looksBlocked` heuristic (CAPTCHA / 418 / 429 / "unusual traffic" / tiny output) to fall through. This dissolves the old `web_search`/`web_fetch` split — one tool, no API keys. *(Updated 2026-07-23: DDG's JS site and Google/Bing intermittently bot-wall the headless browser; DDG's **HTML** endpoint — clean title/url/snippet results below a region-selector header — proved the most reliable, so it's now primary with Bing as fallback. See the search-reliability fix.)*
