...
test("todo registers todo and /pi-todo", async () => {
  const api = await loadExtension("todo");
  expect([...api.tools.keys()]).toEqual(["todo"]);
...
test("todo emits todo:updated and todo:task-complete", async () => {
  const api = await loadExtension("todo");
  const tool = api.tools.get("todo")!;
  const ctx = fakeCtx();
...
test("todo paints the widget", async () => {
  const api = await loadExtension("todo");
  const ctx = fakeCtx();
  await api.tools
    .get("todo")!
