...
test("no reminder without an objective, or once it is met", () => {
  expect(goalReminder(null, null)).toBeNull();
  expect(goalReminder({ ...active, status: "met" }, null)).toBeNull();
});

test("the reminder names the objective and how to close it out", () => {
  const msg = goalReminder(active, null)!;
...
test("the reminder carries the criteria and the todo progress when there are any", () => {
  const msg = goalReminder({ ...active, criteria: "tokens rotate cleanly" }, { done: 2, total: 5 })!;
...
test("the reminder omits progress when nothing publishes it", () => {
  expect(goalReminder(active, null)).not.toContain("todos done");
});
